...
/*
儘管與其他 BST 中採用的刪除方法有相似之處，但該樹結構中的刪除過程相對更加複雜。
刪除節點時，如果要刪除的節點有右子節點，則刪除過程需要將要刪除的節點替換為右子樹中遇到的第一個節點。
在此替換之後，將對新插入的節點的右子節點調用更新操作。

類似地，如果要刪除的節點沒有右子節點，則替換過程涉及利用在左子樹中找到的第一個節點。
隨後，對替換節點的左子節點調用更新操作。

對於待刪除節點沒有子節點（既不左也不右）的場景，可以直接將其從樹中移除，並對被刪除節點的父節點調用更新操作。
*/
